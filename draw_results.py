...
for algo in algnames:
  ...
  resourceactual[algo] = np.empty([len(resource), len(capacity), number_of_experiments])
  ...
  for res in range(len(resource)):
    for cap in range(len(capacity)):
      ...
      for index, algo in enumerate(algnames):
        ...
        resourceuse[algo][res, cap, expno] = round(used/int(number_of_trees*number_of_sources*capacity[cap]), 3)
...
colors = ['k', 'blue', 'red', 'c']
hatches = ['//', r'\\', '..','x']
tick_label = [r"$①\alpha \leq 0.5;\beta \leq 0.5$",
...
for metric in metrics:
  if metric == "compression":
    ...
    for algo in algnames:
      ...
      p = ax.bar(x+bar_width*count, y, bar_width, fill=False, edgecolor=colors[count], hatch=hatches[count], label=algo)
    ...
    plt.savefig(os.path.join(figure_dir, f"{plot_which}-compression." + pic_type), dpi=600, bbox_inches='tight')
    ...
    for algo in algnames:
      y = []
      y.append(100*(1-np.mean(compression[algo][0:datamid, 0:datamid]))/np.mean(resourceactual[algo][0:datamid, 0:datamid]))
      y.append(100*(1-np.mean(compression[algo][0:datamid, datamid:datalen]))/np.mean(resourceactual[algo][0:datamid, datamid:datalen]))
      # y.append(np.mean(compression[topo][algo][5, 5, :]))
      y.append(100*(1-np.mean(compression[algo][datamid:datalen, 0:datamid]))/np.mean(resourceactual[algo][datamid:datalen, 0:datamid]))
      y.append(100*(1-np.mean(compression[algo][datamid:datalen, datamid:datalen]))/np.mean(resourceactual[algo][datamid:datalen, datamid:datalen]))
    ...
    plt.savefig(os.path.join(figure_dir, f"{plot_which}-efficiency."+pic_type), dpi=600, bbox_inches='tight')
