...
for topo in ["fattree", "bcube", "torus", "random"]: # , "torus", "bcube", "random"
  ...
  for index, alg in enumerate(algnames):
    p = ax.bar((bar_width)*index + bar_width/2 + last_position, sum(theta[alg])/len(theta[alg]), bar_width, fill=False, edgecolor=colors[index], hatch=hatches[index], label=alg)
    ax.bar_label(p, fmt='%.2f', size=10, padding=1, label_type='edge')

  last_position = (bar_width)*index + bar_width/2 + last_position + space
ax.set_xticks([bar_width*2, space+bar_width*5.5, 2*space+bar_width*9, 3*space+bar_width*12], topos)
...
for h, l in zip(handles, labels):
  if l not in visited:
    ...
    visited.add(l)
leg = ax.legend(unique_handles, unique_labels, fontsize=10, loc='upper center', ncol=4,
