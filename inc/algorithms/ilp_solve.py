...
def binary_ilp_solve(sources, targets, S, arcs, capactity, M):
    ...
    x = model.addVars(
        keys, arcs, vtype=GRB.INTEGER, lb=0, name="x"
    )  # flow may be compressed
    f = model.addVars(
        keys, arcs, vtype=GRB.INTEGER, lb=0, name="f"
    )  # flow cannot be compressed
    a = model.addVars(
        keys, S, vtype=GRB.BINARY, name="a"
    )  # if allocate memory to key k in node i
    ...
    for k, S0, target in zip(keys, sources, targets):
        ...
        model.addConstrs(
            (f.sum(k, i, "*") == 1 for i in S0),
            f"key_{k}_noncompressed_flow_must_out_at_S0",
        )
        model.addConstrs(
            (f.sum(k, i, "*") - f.sum(k, "*", i) == 0 for i in Sf),
            f"key_{k}_noncompressed_flow_must_conserve_at_Sf",
        )
        model.addConstr(
            (-f.sum(k, "*", target) == -len(S0)),
            f"key_{k}_noncompressed_flow_must_end_at_target",
        )

        model.addConstrs(
            (x.sum(k, i, "*") - x.sum(k, "*", i) + xloop[k, i] == 0 for i in Sf),
            f"key_{k}_flow_aggregate_with_xloop",
        )
        model.addConstrs(
            (x.sum(k, i, "*") - x.sum(k, "*", i) == 1 for i in S0),
            f"key_{k}_flow_aggregate_without_xloop",
        )

        model.addConstrs(
            (xloop[k, i] <= len(S0) * a[k, i] for i in S), f"key_{k}_turnon_selfloop"
        )
        model.addConstrs(
            (xloop[k, i] >= a[k, i] for i in S), f"key_{k}_mustbeusedasBlue"
        )

        model.addConstrs(
            (len(S0) * x[k, i, j] >= f[k, i, j] for i, j in arcs),
            f"key_{k}_force_x_on_chosen_edge1",
        )
        model.addConstrs(
            (x[k, i, j] <= f[k, i, j] for i, j in arcs),
            f"key_{k}_force_x_on_chosen_edge2",
        )
    model.addConstr(
        quicksum(x[k, i, j] for k in keys for i, j in arcs) <= 15, "upbound"
    )
...
def ilp_solve_traffic(
    sources, targets, S, arcs, capactity, M, silence=False, reuse=True
):
    ...
    if not reuse:
        ...
        for k, S0, target in zip(keys, sources, targets):
            ...
            model.addConstrs(
                (xloop[k, i] <= len(S0) * a[k, i] for i in S),
                f"key_{k}_turnon_selfloop",
            )
            model.addConstrs(
                (xloop[k, i] >= a[k, i] for i in S), f"key_{k}_mustbeusedasBlue"
            )
        ...
        model.setObjective(
            quicksum(x[k, i, j] for k in keys for i, j in arcs), gp.GRB.MINIMIZE
        )
        model.optimize()
...
def ilp_based_on_edge(
    ...
    if not reuse:
        ...
        x = model.addVars(
            arcs, key_range, vtype=GRB.INTEGER, lb=0, name="x"
        )  # flow may be compressed
        ...
        f = model.addVars(
            arcs, key_range, vtype=GRB.INTEGER, lb=0, name="f"
        )  # flow cannot be compressed
        ...
        model.addConstr(
            (quicksum(a[i, k] for i in Sf for k in key_range) <= M), "job_memory"
        )
        # model.addConstrs(
        ...
        model.addConstrs(
            (f.sum(i, "*", k) - f.sum("*", i, k) == 1 for i in S0 for k in key_range),
            "noncompressed_flow_conservation_f_S0_Sf",
        )
        model.addConstrs(
            (f.sum(i, "*", k) - f.sum("*", i, k) == 0 for i in Sf for k in key_range),
            "noncompressed_flow_conservation_f_S0_Sf",
        )
        model.addConstrs(
            (
                f.sum(target, "*", k) - f.sum("*", target, k) == -len(S0)
                for k in key_range
            ),
            "r_nodes",
        )
        ...
        model.addConstrs(
            (
                x.sum(i, "*", k) - x.sum("*", i, k) + xloop[i, k] == 0
                for i in Sf
                for k in key_range
            ),
        ...
        model.addConstrs(
            (xloop[i, k] <= Theta * a[i, k] for i in Sf for k in key_range),
            "turnon_selfloop",
        )

        model.addConstrs(
            (Theta * x[i, j, k] >= f[i, j, k] for i, j in arcs for k in key_range),
            "force_x_on_chosen_edge1",
        )
        model.addConstrs(
            (x[i, j, k] <= Theta * f[i, j, k] for i, j in arcs for k in key_range),
            "force_x_on_chosen_edge2",
        )
        model.setObjective(
            quicksum(x[i, j, k] for i, j in arcs for k in key_range), gp.GRB.MINIMIZE
        )
        model.optimize()
...
def ilp_solve(
    ...
    total_traffic, a, x, xloop, f_all = ilp_solve_traffic(
        sources, targets, S, arcs, capacity, M, silence=not Debug, reuse=reuse
    )
    ...
    if show_aggr_graph:
        ...
        if show_path_graph:
            ...
            fig.canvas.manager.set_window_title(
                "Aggregation Graph and Path for each Source"
            )
        else:
        ...
        for k in range(K):
            ...
            target = targets[k]
            Sf = [n for n in S if n not in S0 and n != target]
            color_node, node_color = get_node_color(
                S0, Sf, target, key_compution_nodes[k]
            )
...
def get_branches(G, k, sources, target, x, aggr_nodes):
    ...
    S0 = sources
    aggr_inflows_num = {
        aggr: sum([x[k, i, j] for i, j in arcs if j == aggr]) for aggr in aggrs
    }
...
def get_node_color(S0, Sf, target, key_compution_nodes):
    ...
    color_node["xkcd:white"] = list(
        set(S0 + Sf + [target])
        - set(itertools.chain.from_iterable(color_node.values()))
    )  # the rest nodes are white
