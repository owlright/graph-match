...
def get_tree(G:nx.DiGraph, sources:list, root:int, algorithm="avalanche",weight=None):
...
def tree_soar(G:nx.DiGraph, sources:list, target:int, L, M, algorithm="avalanche", weight=None):
  '''
  Parameters
  ----------
  ...
  for u in source_switches: # sum up all source hosts load on the leaf swtich
    G.nodes[u]['load'] = sum([G.nodes[i]['load'] for i in G.predecessors(u) if G.nodes[i]['type']=='host'])
  ...
  for i in range(L):
    ...
    for u in blue_nodes: # ! must remove the used memory
      results[u] = results.get(u, 0) + 1 # switch u allocate 1 unit memory to this key
      G.nodes[u]['capacity'] -= 1
      if G.nodes[u]['capacity'] == 0 : 
        logger.debug(f"{u} is exhaust")
        need_recalc = True # TODO for now, only when the capacity==0 then recalc the tree 
        # G.remove_node(u) 
        # plot(G)
    if need_recalc:
