...
def soar_gather(T: nx.DiGraph,
                k,
                Debug=False) -> int:  # k is the number of blue nodes
  T_succ = T._succ
  T_pred = T._pred
  X = {}
  Y = {}
  ...
  for d in range(levels, -1, -1):
    ...
    for v in nodes:
      ...
      if len(childs) == 0:
        ...
        X[v] = np.zeros((d + 1, k + 1))
        children[v] = childs # record the children for reverse use in soar-color
        ...
        for m, n in enumerate(childs):
          for l in range(d + 1):  # distance to destation or blue node
            for i in range(k + 1):
              if m == 0:
                Y[v][m][l][i][BLUE] = \
                  X[n][1][i-1] + rho(T_succ, v, l) if i > 0 else np.inf
                Y[v][m][l][i][RED] = \
                  X[n][l+1][i] # ! not sure about this + rho(T, v, l)*T.nodes[v]['load']
              else:
                Y[v][m][l][i][BLUE] = \
                  mCost(l, i, Y[v][m-1,:,:,:], X[n], BLUE) \
                    if capacity[v] > 0 and i > 0 else np.inf
                Y[v][m][l][i][RED] = \
                    mCost(l, i, Y[v][m-1,:,:,:], X[n], RED)
...
def soar_color(T: nx.DiGraph, k, X, Y, children, Debug=False):
  ...
  T.add_node(root, color=colors['red'], l=1,
             num_blues=k)  # start from the root switch to bottom switch
  for d in range(1, levels + 1):
    ...
    for v in nodes:
      ...
      if (nChildren > 0):
        ...
        if Y[v][-1, l, nB,colors['blue']]\
        < Y[v][-1, l, nB,colors['red']] and T.nodes[v]['capacity'] > 0:
        ...
        for m, c in enumerate(reversed(
            children[v])):  # children in reverse order
          m = nChildren - m - 1
          if m == 0: break
          j = mSplit(l, nB, Y[v][m - 1, :, :, :], X[c],
                     T.nodes[v]['color'], Debug)
          if Debug:
  ...
  for k, T in enumerate(graphs):
    for node in T:
      ...
      tree.add_node(node_name,
                    key=k,
                    node=node,
                    capacity=1 if capacity[node] > 0 else 0)
    ...
    # while invalid_nodes:
      ...
      for k in blue_keys[node]:
        ...
        for _ in range(bluel-1):
          ...
          if parent_node == t:
            break
        nobluel = bluel + tree.nodes[parent_node]['l'] if parent_node != t else bluel
        # aggr_traffic.append(tc)
        add_traffic_if_no_this_blue = X[node_name][nobluel][len(key_blues[k])] -\
        ...
        heappush(q, (add_traffic_if_no_this_blue, k)) # todo why not do like my mat algorithm does?

      while overflow: # ! set the most un-affected tree capcacity this node to zero
        _, key = heappop(q)
...
def case2():
  ...
  for m in M:
    ...
    traffic = mtree_soar(graphs, G.copy(), sources.copy(), targets.copy(), capacity.copy(), m)
