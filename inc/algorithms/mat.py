    ...
    while M != 0:
        for k in deal_keys:
            ...
            for a in Ws[k]:  # try each potential aggr nodes
                mst = mstnodes + [a]
                parr = extract_paths_njit_nodict(np.array(S, dtype=np.int32), np.array(mst, dtype=np.int32), oddist)
                i = 0
                pdict = {}
                cost = 0
            ...
            if minNode is not None:  # * the best inc node that can reduce most cost of kth tree
                heappush(candiate_aggrs, (minCost, k, minNode, minP))
            else:  # * this tree doesnt need aggregation nodes any more
                continue

        if not candiate_aggrs:  # ! this will happen if M is too big that no more tree needs
            break
        reducedCost, key_to_change, aggr_to_add, paths_to_replace = heappop(candiate_aggrs)
...
def mat(sources, targets, steiner_trees, capacity, M, oddist, odpath, Debug=False):
    ...
    while M != 0:
        for k in deal_keys:
            ...
            for a in potential_aggrs[k]:  # try each potential aggr nodes
                ...
                if joint_node != r:
                    ...
                    to_tree_direction = oddist[a, joint_node] + oddist[joint_node, P[joint_node]]

                    from_tree_direction = oddist[joint_node, a] + oddist[a, second_joint_node]
                ...
                if cost - last_cost < minCost:
                    ...
                    minP = tmp
            # ! node may be 0, you have to write not None here
            if minNode is not None:  # * the best inc node that can reduced most cost of kth tree
        ...
        if not candiateIncNodes:  # ! this will happen if M is too big that no more tree needs
            break
        reducedCost, key_to_change, aggr_to_add, paths_to_replace = heappop(candiateIncNodes)
        # tree = construct_tree_from_paths(paths_to_replace, odpath)
        if Debug:
            print(f"choose key {key_to_change} set aggr {aggr_to_add}")
        # * update the chosen key's Paths
        for s, t in paths_to_replace.items():
            for n in odpath[s, t]:
                if n not in forbiddens and n not in As[key_to_change]:
                    potential_aggrs[key_to_change].add(n)  # ! find more potential aggregation nodes
        ...
        deal_keys = [key_to_change]  # ! only the changed key tree need to recalc it cost again
        # * update the resource use
        affect_keys = update_resource(aggr_to_add, capacity, candiateIncNodes, forbiddens)
        deal_keys = deal_keys + affect_keys
        M = M - 1
    cost = 0
    for k in range(K):
...
def test_fattree():
    ...
    traffic = mat(G.copy(), sources.copy(), targets.copy(), capacity.copy(), 4, Debug=True)
