...
def camdoop(G: nx.Graph, S, r, n) -> nx.DiGraph:
    ...
    node2xyz = {
        node: G.nodes[node]["pos"] for node in G if G.nodes[node]["type"] == "switch"
    }
    xyz2node = {
        G.nodes[node]["pos"]: node for node in G if G.nodes[node]["type"] == "switch"
    }
    ...
    aggr_graph._node.update(
        (n, d.copy()) for n, d in G.nodes.items() if n in aggr_graph
    )
    return aggr_graph
