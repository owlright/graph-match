...
def irs_based(G: nx.Graph, S, r, k, DEBUG=False) -> nx.DiGraph:
    ...
    for j in range(stage_num, 0, -1):  # stage_num..1
        ...
        for i in range(k + 1):  # only dimension i is different
            ...
            for server in servers:
                ...
                if server != tuple(aggr):
                    ...
                    Found = False
                    for s in stage_server[
                        j - 1
                    ]:  # check if there is a neighbor in next stage
        ...
        routing_symbol[chosen_index] = (
            True  # * this dimension now is the same with root server
        )
        if DEBUG:
            print(
                f"choose routing {chosen_index}, aggr servers: ",
                list(chosen_server_aggr.values()),
            )
        ...
        for sender, aggr in chosen_server_aggr.items():
            ...
            aggr_graph.add_edge(
                pos2switch[aggr_switch], pos2server[aggr]
            )  # may be add multiple times but its ok
...
def test_basic():
    ...
    for j in range(d, 0, -1):
        ...
        print(
            "through switches:",
            [get_connected_switch(s, chosen_index) for s in chosen_servers],
        )
