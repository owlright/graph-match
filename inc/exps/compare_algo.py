...
def compare_algo(result_file_name:str, topo:nx.Graph, number_of_experiments, Debug=True, silent=True):
  ...
  for expno in range(number_of_experiments):
    ...
    for treeshare_ratio in richness:
      for swichcapacito_ratio in capacity_ratio:
        ...
        for algo in greedy_algos:
          if algo == "GCAT":
            traffic, Ps, As = gstep(sources, targets, capacity.copy(), memory_canused, oddist, odpath)
          else:
            if algo == "Shortest": # * tree algos
              ...
              trees = [camdoop(G_reindexed, S, r, 8) for S, r in zip(sources, targets)]
            elif algo == "IRS":
              trees = [irs_based(G_reindexed, S, r, 3, False) for S, r in zip(sources, targets)]
            elif algo == "Avalanche":
              trees = [avra(G_reindexed, S.copy(), r) for S, r in zip(sources, targets)]
            else:
              assert False
            traffic, Ps, As = greedy_tree(sources, targets, trees, capacity.copy(), memory_canused, oddist, odpath)
          # ! actual resources algorithm used because
          # ! when M is redunant some resource will not be used.
          num_of_comp_nodes = sum([len(As[k]) for k in range(number_of_trees)])
    ...
    print(f"{topo} rest time(min): {(sum(exp_time) / len(exp_time)) *(number_of_experiments - 1 - expno) / 60:.3f}")
  result_file.close()
