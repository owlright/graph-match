...
def compare_ideal(result_file_name, topo, number_of_experiments, Debug=False, silent=True):
  ...
  for expno in range(number_of_experiments):
      ...
      for index, alg in enumerate(tree_algos):
        if not enoughs[index]:
          if alg == "OPT":
            ...
            cost, Ps, As = greedy_tree([S], [r], [shortest_tree], capacity.copy(), m, oddist, odpath)
            P = Ps[0]
            A = As[0]
          elif alg == "Takashami":
            if topo != "bcube":
              steiner_tree = build_steiner_tree(S, r, oddist, odpath, set(hosts))
            else:
              steiner_tree = build_steiner_tree(S, r, oddist, odpath)
            cost, Ps, As = greedy_tree([S], [r], [steiner_tree], capacity.copy(), m, oddist, odpath)
            P = Ps[0]
            A = As[0]
          elif alg == "GCAT":
            cost, Ps, As = gstep([S], [r], capacity.copy(), m, oddist, odpath)
            P = Ps[0]
            A = As[0]
          elif alg == "IRS":
            tree = irs_based(G, S, r, network_level)
            cost, Ps, As = greedy_tree([S], [r], [tree], capacity.copy(), m, oddist, odpath)
            P = Ps[0]
            A = As[0]
          elif alg == "Camdoop":
    ...
    print(f"{topo} used:{used/60} mins, estimated:{used*(number_of_experiments-expno-1)/60} mins")
    print()
  result_file.close()
