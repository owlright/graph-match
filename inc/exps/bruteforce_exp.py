...
def bruteforce_exp(S, r, capacity, M, oddist, Debug=False):
    ...
    S_share[:] = Sarr
    tmp3 = shared_memory.SharedMemory(
        size=oddist.nbytes, create=True, name=thispid + "oddist"
    )
    ...
    tmp4 = shared_memory.SharedMemory(
        create=True, size=pieces.nbytes, name=thispid + "otherinfo"
    )
    ...
    results = pool.imap(
        heavy_comp, itertools.combinations(paggrs, M), chunksize=chunksize
    )
