...
def fattree(k, singleHost=True):
  ...
  num_total_nodes = num_hosts+num_edge_switches+num_agg_switches+num_core_switches
  ...
  for i in range(start_index, start_index+num_edge_switches):
    ...
    hosts_per_edge = k//2
    connected_hosts = [j for j in range((i-count)*hosts_per_edge, (i-count)*hosts_per_edge+hosts_per_edge)]
  ...
  for i in range(start_index, start_index+num_agg_switches):
    ...
    for j in [x for (x, y) in F.nodes(data=True) if y['level']==1 and y['pod']==F.nodes[i]['pod']]:
  ...
  for i in range(start_index, start_index+num_core_switches):
    ...
    for j in [x for (x, y) in F.nodes(data=True) if y['level']==2 and y['group']==F.nodes[i]['group']]:
      F.add_edge(i, j) # conncet the aggr switch to edge switches
    start_index += 1
  for (u, v) in F.edges():
...
if __name__ == "__main__":
  ...
  for (u, v) in F.edges():
      F.edges[u,v]['weight'] = random.randint(0,10)
  # for i in range(F.number_of_nodes()):
  #   print(F.nodes[i])


  fig, ax = plt.subplots()

  solution_graph = F.copy()
  terminal_nodes = [1, 3, 4, 11, 15]